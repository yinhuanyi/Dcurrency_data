...
from utils.const_file import  BINANCE_INFO_LOG, BINANCE_INFO_LOG

binance_logger = getlogger('binance', BINANCE_INFO_LOG, BINANCE_INFO_LOG)
...
class BiWetsocket:
    ...
    def on_open(self, spot_list):
        ...
        for spot in spot_list:
            trade_list.append('{}@trade'.format(spot))
    ...
    def on_msg(self, data: str):
        ...
        if data.get('stream'):
            # print(data)
            spot_name = data.get('stream').split('@')[0]
            price_data = data.get('data')
            spot_price = price_data.get('p')

            binance_logger.info('{}={}'.format(spot_name, spot_price))
...
def get_spot(spot_list):
    baseurl = 'wss://stream.binance.com:9443'
    multi_stream = '/stream?streams={}'.format('/'.join(spot_list))
    wss_url = baseurl + multi_stream
    okex_ws = BiWetsocket(host=wss_url, ping_interval=20)
    okex_ws.start(spot_list)


if __name__ == '__main__':
    # 这个数据从数据库里面读取
    spot_list = ['btcusdt', 'ethusdt', 'ankrusdt']
    # 开启多个线程来获取数据
